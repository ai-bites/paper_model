...
global figure_model sh current_plot current_point sfPoint current_ruling



if ~isnull(ishandle(current_plot))
    delete(cp(ishandle(current_plot)));
end
figure(figure_model)
...
[nth,ind] = min(v);
current_point = I(:,ind);
current_plot = [current_plot plot(current_point(1),current_point(2),'r.','markersize',10)];
if isequal(sfPoint,'s')
